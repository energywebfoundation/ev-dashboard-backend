...
export class FlexController {
  ...
  constructor(
    ...
    @inject(ASSET_ACTIVATION_SERVICE)
    private assetActivationService: AssetActivationService,
  ) {
  ...
  async resetDatabase(): Promise<String> {
    return this.assetRepository.find().then(assets => {
      assets.forEach(asset => {
        this.client.publish(
          'asset/' + asset.serialNumber + '/control',
          JSON.stringify({command: 'deregister'}),
        );
      });
      return Promise.all([
  ...
  async testGenerateMerkleRoot(): Promise<string> {
    const constraints = await this.constraintsRepository.find();
    console.log(constraints);
    this.offerRepository
      .find({
        where: {offerType: OFFER_TYPE.SUPPLY},
        order: ['price asc'],
        include: [{relation: 'asset'}],
      })
      .then(recentOffers => {
        // console.log(constraints);
        ...
        recentOffers.forEach((recentOffer: any) => {
          ...
          let constraint: any;
          if (
            recentOffer.asset.serialNumber ==
            CONSTRAINTS['a9c30590-93cb-4e0b-bf0b-e59fdb96110b']
          ) {
            constraint = find(constraints, ['constraints', 'MP1']);
          } else if (
            recentOffer.asset.serialNumber ==
            CONSTRAINTS['2a39f69e-ddab-42d3-b37b-87ac52ef15a1']
          ) {
            constraint = find(constraints, ['constraints', 'MP1']);
          } else if (
            recentOffer.asset.serialNumber ==
            CONSTRAINTS['60a11da9-20b8-47bc-b365-a49d98277d05']
          ) {
            constraint = find(constraints, ['constraints', 'MP2']);
          } else if (
            recentOffer.asset.serialNumber ==
            CONSTRAINTS['c80b9a03-7009-40ef-987d-c7058b60c844']
          ) {
            constraint = find(constraints, ['constraints', 'MP2']);
          }
  ...
  async sentCommandIot(@requestBody() command: any): Promise<void> {
    console.log(command);
  ...
  async replaceById(@requestBody() offer: any): Promise<void> {
    console.log('99999999');
    console.log(offer);
    this.client.publish(
      offer.asset.serialNumber + '/offer/accept',
      JSON.stringify({
        status: 'accept',
  ...
  async reserveBundle(): Promise<void> {
    console.log('Trigger offer bundle By REST');
    this.offerRepository
      .find({
        where: {
          offerType: OFFER_TYPE.DEMAND,
          offerStateId: OFFER_STATE.PENDING,
        },
      })
      .then(results => {
        results.forEach(demandOffer => {
          ...
          if (demandOffer.offerStateId == OFFER_STATE.PENDING) {
            const d = new Date(demandOffer.date);
            const n = Math.ceil(
              ((d.getHours() * 60 + d.getMinutes()) / 60) * 4,
            );
            ...
            const _recentOffers: Array<Offer> = [];
            this.offerRepository
              .find({
                where: {
                  offerType: OFFER_TYPE.SUPPLY,
                  offerStateId: OFFER_STATE.PENDING_SUBMIT,
                },
                order: ['price asc'],
              })
              .then(recentOffers => {
                ...
                recentOffers.forEach(recentOffer => {
                  console.log(recentOffer);
                  const _d = new Date(recentOffer.date);
                  const _n = Math.ceil(
                    ((_d.getHours() * 60 + _d.getMinutes()) / 60) * 4,
                  );
                  console.log(n + ' ' + _n);
                  console.log(d.toLocaleString() + ' ' + _d.toLocaleString());

                  if (
                    recentOffer.offerStateId == OFFER_STATE.PENDING_SUBMIT &&
                    _n == n &&
                    d.getFullYear() == _d.getUTCFullYear() &&
                    d.getUTCMonth() == _d.getUTCMonth() &&
                    d.getUTCDay() == _d.getUTCDay() &&
                    (demandOffer.capacity > 0 ? '+' : '-') ==
                      (recentOffer.capacity > 0 ? '+' : '-')
                  ) {
                    console.log(recentOffer);
                    const positiveCapacity =
                      Number(recentOffer.capacity) < 0
                        ? Number(recentOffer.capacity) * -1
                        : Number(recentOffer.capacity);
  ...
  async BidOrder(
    ...
    this.client.publish(
      bidOrder.did.serialNumber + '/offer/create',
      JSON.stringify(bidOrder.offer),
    );
  ...
  async ActivationRequest(
    ...
    return new Promise(resolve => {
      this.offerBundleRepository
        .offers(activationRequestBody.activationRequest.reservedBundleId)
        .find({
          where: {
            offerType: OFFER_TYPE.SUPPLY,
            offerStateId: OFFER_STATE.CONFIRM,
          },
          order: ['price asc'],
        })
        .then(async result => {
          ...
          console.log(result);
          const offerBundle: OfferBundle = await this.offerBundleRepository.findById(
            Number(activationRequestBody.activationRequest.reservedBundleId),
          );
          console.log(offerBundle);
          const activationBundle: any[] = [];
          result.forEach(async (offer: any, index) => {
            const asset: Asset = await this.assetRepository.findById(
              Number(offer.assetId),
            );
        ...
        .then(activationBundle => {
          this.assetActivationService.startActivation(
            this.client,
            activationBundle,
          );
          resolve();
  ...
  async ownerCarned(@param.path.number('id') id: number): Promise<any> {
    return new Promise(resolve => {
      ...
      this.offerRepository
        .find({
          where: {ownerId: id},
          include: [{relation: 'activationSummary'}],
        })
        .then(result => {
          // console.log(result);
          ...
          result.forEach(offer => {
            ...
            if (offer.activationSummary?.summary) {
              ...
              const values = JSON.parse(
                JSON.stringify(offer.activationSummary?.summary),
              );
  ...
  async ownerRecentActivation(
    @param.path.number('id') id: number,
  ): Promise<any> {
    return new Promise(resolve => {
      this.offerRepository
        .findOne({
          where: {ownerId: id, offerStateId: OFFER_STATE.CONFIRM},
          order: ['id desc'],
          include: [{relation: 'activationSummary'}],
        })
        .then(offer => {
