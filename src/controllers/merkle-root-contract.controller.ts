...
export class MerkleRootContractController {
  ...
  public getMR(
    ...
    @param.query.string('timestamp') timestamp: string,
  ): Promise<any> {
    return this.connector.contract.methods
      .getMR(flexRequestID, timestamp)
      .call({from: config.options.defaultAccount});
  }
