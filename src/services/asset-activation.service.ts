...
@bind({scope: BindingScope.SINGLETON,tags: {[ContextTags.NAMESPACE]: 'services'}})
export class AssetActivationService {
  ...
  public startActivation(mqttInstance: any, offerAssets: any[]):void {
    ...
    let websocketInstance = this.webSocket;    
          ...
          offerAssets.forEach(offerAssetItem => {       
            ...
            if (offerAssetItem.offerCapacity >= 0 ){
              if(totalCapacity + (Number(offerAssetItem.offerCapacity)) >= 0){
                ...
                mqttInstance.publish(offerAssetItem.assetId + '/offer/activate', JSON.stringify({            
                  ...
                  "baseline": (totalCapacity >= 0)?offerAssetItem.offerCapacity:totalCapacity + offerAssetItem.offerCapacity
                }));
                // console.log({
                //   "offerID": offerAssetItem.offerId,
                //   "assetID": offerAssetItem.assetId,    
